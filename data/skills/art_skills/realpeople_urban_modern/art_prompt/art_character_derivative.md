...
> **架构决策**：T0 底模为 **21:9 四视图 turnaround**（脸锚）；T1 服化为 **16:9 单图服化参考**（同脸 + 服装差异），**不是**四视图衍生。生图时自动引用 T0 图作 `referenceList`。
...
|---|---|
| 画幅 | **16:9** 横版单图 |
| 构图 | 半身或全身立像，正面或微侧，自然站立 |
| 背景 | 纯净中性灰 #E8E8E8 |
| 站姿 | 自然站立、双脚平行微分、双臂自然下垂 |
| 表情 | 符合妆容风格的微表情，仅限面部微表情 |
| 光线 | 均匀柔光，前方主光 + 双侧补光，无硬阴影 |
| 一致性 | 面容/妆容/发型与 T0 底模完全一致，仅服化妆造可变 |
| 参考图 | **必须**引用 T0 四视图底模作 img2img / reference |

### 与 T0 四视图的分工

| 层级 | 画幅 | 用途 |
|---|---|---|
| T0 底模 | 21:9 四视图 | 脸型锚点、五官/发型基准 |
| T1 服化 | 16:9 单图 | 日常/潜入/落魄等服化变体参考 |
...
```
以角色 T0 四视图底模为参考图，img2img 叠加服化妆造，
都市{性别}角色服化单图参考，16:9，真人写实摄影，都市写实纪实，强对比度，极致细节，8K，超保真
single wardrobe reference，保持基础形象面容不变，{整体气质}，
...
| R3 | 妆容/发型/服饰/配饰风格统一 |
| R4 | 必须输出 **16:9 单图服化参考**（非四视图） |
| R5 | 必须指定「纯净中性灰背景」 |
| R6 | 必须引用 T0 底模作 reference / img2img |
...
| X14 | **输出四视图 turnaround**（四视图仅属于 T0 底模） |
## 八、T1 服化单图规范（16:9）
...
|------|------|------|
| T0 底模 | 21:9 四视图 | 脸型锚点 |
| T1 服化 | 16:9 单图 | 服化变体参考 |

禁止 T1 输出四视图 turnaround；脸锚由 lock face + T0 reference 承担。
