...
import { b as preflightTouch } from './ruleEngine-BPsMqBhU.js';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    async function runPreflight() {
      ...
      try {
        const selectedSbId = (() => {
          const fromList = imageList.value.find((i) => i.sources === "storyboard" && typeof i.id === "number")?.id;
          if (fromList != null) return fromList;
          const fromTrack = currentTrack.value?.medias?.find((m) => m.sources === "storyboard" && typeof m.id === "number")?.id;
          return fromTrack ?? null;
        })();
        ...
        if (!result.allowed) {
          const blockMsg = result.report?.issues?.find((i) => i.severity === "BLOCK")?.message || "";
          window.$message.error($t("workbench.production.rulePanel.preflightBlock") + (blockMsg ? `: ${blockMsg}` : ""));
          return false;
        }
        return true;
      } catch {
...
const index = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-fe63f766"]]);
