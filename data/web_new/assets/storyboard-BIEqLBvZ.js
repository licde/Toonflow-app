...
import { e as editImage } from './index-BnXcbjqI.js';
...
import { a4 as Empty, a7 as CheckboxGroup, B as Button, a3 as Checkbox, X as Tag, a8 as Image, L as Loading, n as Tooltip, ac as InputNumber, G as ImageViewer, W as DialogPlugin, s as LoadingPlugin, Y as Card, T as Textarea } from './tdesign-CfL1pweZ.js';
import { _ as _export_sfc } from './index-CYEzD5Ot.js';
import './dayjs-CuToSpIM.js';
import './assetsCheck-C_EAKhUb.js';
import './index-yBIIU3Xk.js';
import './modelSelect-C4MDdN3m.js';
import './providersLogo-BCbaFq8_.js';
import './promptEditor-Cb3f-aQB.js';
import './icons-B-vHNScY.js';
import './index-VjQmDwQW.js';
import './markdown-CDQfeHxT.js';
import './i18n-C05S5xzz.js';

const CREF_RE = /--cref\s+([^\s,，]+)/gi;
const SREF_RE = /--sref\s+([^\s,，]+)/gi;
...
function parsePromptRefs(prompt) {
  const crefs = [];
  const srefs = [];
  for (const m of prompt.matchAll(CREF_RE)) crefs.push(normalizeCode(m[1]));
  for (const m of prompt.matchAll(SREF_RE)) srefs.push(normalizeCode(m[1]));
  ...
  setup(__props) {
    ...
    function resolveAssetByCode(code) {
      ...
      return props.assetsData.find(
        (a) => a.name === code || a.name?.includes(suffix) || a.prompt?.includes(code) || a.desc?.includes(code)
      );
    }
...
const storyboard = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-691a70aa"]]);
