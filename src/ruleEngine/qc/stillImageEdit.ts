...
export function buildEditFocusPrompt(input: {
  ...
  // 1) Contact geometry — XOR-aware: dual cheek+oral → inject mutual exclusion, skip oral-locus geom that collapses to 咬纸
  let xorDualContact = false;
  try {
    const { auditLiteraryDetailQuality, extractDeclaredContactLoci, hasContactRoleXorSatisfaction } =
      require("../compilers/stillLiteraryDetailQuality") as typeof import("../compilers/stillLiteraryDetailQuality");
    const a = auditLiteraryDetailQuality({ visualDescription: vd });
    xorDualContact = a.findings.some((f) => f.id === "DEX-LIT-CONTACT-XOR" && f.severity === "BLOCK");
    if (xorDualContact || (!hasContactRoleXorSatisfaction(vd) && /划过|贴/.test(vd) && /咬|渗血|血珠/.test(vd))) {
      priority.push("互斥：纸未入口；颊触与口创不同时含纸咬唇；纸角在面颊外侧划过");
      xorDualContact = true;
    }
    for (const locus of extractDeclaredContactLoci(vd).slice(0, 2)) {
      if (!locus || FACING_ONLY_LOCUS.test(locus)) continue;
      // Skip oral loci while XOR unresolved — geom heal on 下唇 steers model to 咬纸
      if (xorDualContact && /唇|口|嘴/.test(locus)) continue;
    ...
    // Contact-event: must show prop in frame — scar alone forbidden
    try {
      const { matchContactEventVd, isContactEventVd } =
      ...
      if (isContactEventVd(vd)) {
        ...
        const locusLabel = m.locus || "面颊";
        const propLine = `须可见${prop}与${locusLabel}接触，禁止仅浅痕无${prop}`;
        if (!rawHints.some((h) => h.includes(prop) && /须可见|禁止仅浅痕/.test(h))) {
          priority.unshift(propLine);
        }
      }
    } catch {
      /* optional */
