...
function loadChecklist(): { id: string; severity: string }[] {
  const p = path.join(process.cwd(), "data", "fixtures", "design_closure_checklist.json");
  if (!fs.existsSync(p)) return [];
  return JSON.parse(fs.readFileSync(p, "utf-8")).checks ?? [];
}
