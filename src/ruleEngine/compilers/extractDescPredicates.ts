...
function paperDocAliasAlt(): string {
  try {
    const { contactPropAliasAlternation, loadContactEventPolicy } =
      require("./contactEventPolicy") as typeof import("./contactEventPolicy");
    const policy = loadContactEventPolicy();
    const paper = policy.vocab.propClasses.paper_doc?.aliases ?? [];
    if (paper.length) {
      return paper
        .map((a) => a.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))
    ...
    return contactPropAliasAlternation() || "休书|婚书|信笺|信纸|纸角|书信|纸";
  } catch {
    return "休书|婚书|信笺|信纸|纸角|书信|纸";
  }
}
...
/** Verb + optional prop patterns (ordered: more specific first); paper aliases from doctrine. */
function predicatePatterns(): Array<{ verb: string; prop?: string; re: RegExp }> {
  return buildPredicatePatterns();
}
...
export function extractDescPredicates(input: {
  ...
  for (const clause of splitClauses(desc)) {
    for (const pat of predicatePatterns()) {
