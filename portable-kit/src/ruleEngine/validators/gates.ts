...
export function dialogueFidelityGate(
  ...
  script: string,
  opts?: {
    planData?: ScriptBundle["planData"];
    /** When set, only extras on these storyboards fail ok (missing full-plan ignored) */
    storyboardIds?: number[];
  },
): ValidationIssue[] {
  const filtered = opts?.storyboardIds != null;
  const scopedShots =
    filtered
  ...
  const report = dialogueCoverageReport({
    script,
    shots: filtered ? scopedShots : pkg.shots,
    planData: opts?.planData,
    shotScope: filtered ? "filtered" : "full",
  });
    ...
    return [
      {
        ...
        message: formatDialogueCoverageMessage(report, { filtered }),
        rollbackLayer: "SB",
      },
      {
        ruleId: "R2",
