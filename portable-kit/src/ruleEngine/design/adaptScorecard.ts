...
export type AdaptScoreInput = {
  ...
  cuCastOk?: boolean;
  /** Literary detail / XOR resolved (no DEX-LIT-* BLOCK residual) */
  litDetailOk?: boolean;
...
export function scoreAdaptationDesign(input: AdaptScoreInput, passThreshold = 65): AdaptScoreResult {
  ...
  details.lit_detail =
    input.litDetailOk === false ? 0.25 : input.litDetailOk === true ? 0.92 : 0.75;
  if (input.litDetailOk === false) failDims.push("lit_detail");

  details.lit_contact_xor =
    input.litContactXorOk === false ? 0.25 : input.litContactXorOk === true ? 0.92 : 0.75;
  if (input.litContactXorOk === false) failDims.push("lit_contact_xor");
  ...
  const weights: Record<string, number> = {
    ...
    shotIntent: 0.09,
    selfcheck: 0.09,
    contentTranslate: 0.07,
    vis_beat: 0.09,
    cu_cast: 0.07,
    lit_detail: 0.07,
    lit_contact_xor: 0.05,
  };
  let score = 0;
  let w = 0;
