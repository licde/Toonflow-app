...
import { qfExprAdapter } from "./qfExpr";
import { vidContactBeatsAdapter } from "./vidContactBeats";
...
export function ensureDefaultAdapters(): void {
  ...
  if (!adapters.has("QF-EXPR-01")) registerAdapter(qfExprAdapter);
  if (!adapters.has("VID-CONTACT-BEATS")) registerAdapter(vidContactBeatsAdapter);
}
