...
export function normalizePreDesignPack(bundle: ScriptBundle, opts: NormalizePreDesignOpts = {}): NormalizePreDesignResult {
  ...
  // Strip duration-only pseudo dialogue (：3s / ：：：：3s) so DC-01 EXTRA cannot recur
  try {
    ...
    const cleaned = stripNonLiteraryDialogueFromShots(shots);
    if (cleaned.stripped > 0) {
      pack.shots = cleaned.shots as never;
      warnings.push(`strip_duration_dialogue:${cleaned.stripped}`);
      healLog.push("DC-01", "narrative.dialogue.lines", `strip_non_literary:${cleaned.stripped}`);
    }
  } catch {
    /* optional */
  }
