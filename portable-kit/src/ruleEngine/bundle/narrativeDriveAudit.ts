...
export function auditNarrativeDriveGaps(bundle: ScriptBundle): BundleGap[] {
  ...
  if (!ledger.length) {
    gaps.push({
      id: "NAR-01",
      // Episode primary drive: BLOCK when any shot declares contact/propPose chain need (集级闸)
      severity:
        (bundle.preDesignPack?.shots ?? []).some((s) =>
          /休书|划过|贴合|持|握|propPose/.test(String(s.visualDescription ?? "")),
        )
          ? "BLOCK"
          : "WARN",
      message: "缺少 informationLedger（集级主推因果未闭合）",
