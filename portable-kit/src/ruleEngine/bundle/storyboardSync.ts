...
export async function syncStoryboardToDb(
  ...
  for (let i = 0; i < panels.length; i++) {
    ...
    const banIndexMedia =
      ...
      Boolean((panel as { _litXorSplitId?: unknown })._litXorSplitId) ||
      Boolean((panel as { _cuCastSplitId?: unknown })._cuCastSplitId) ||
      Boolean((panel as { xorSplit?: unknown }).xorSplit) ||
      Boolean((panel as { _splitFrom?: unknown })._splitFrom) ||
      Boolean((panel as { _vidSplitId?: unknown })._vidSplitId) ||
      Boolean((panel as { parentStoryboardId?: unknown }).parentStoryboardId) ||
      /_child|_split|子镜/i.test(String((panel as { id?: string }).id ?? ""));
