...
function sanitizeFiveBody(
  ...
  if (audio) {
    ...
    if ((hasDial || bodyHasDial) && silent) {
      ...
      // Empty dialogueLines [] = design confirmed silent → strip orphan CJK too.
      // Undefined dialogueLines = unknown (mode-adapt seed) → keep embedded literary CJK.
      const designConfirmedSilent = Object.prototype.hasOwnProperty.call(input, "dialogueLines");
      const embeddedCjk = [...audio.body.matchAll(/["“]([^"”\n]{2,80})["”]/g)]
        .map((m) => String(m[1] ?? "").trim())
        .filter((t) => t && /[\u4e00-\u9fff]/.test(t) && !/^[:：.\d\s]+$/.test(t) && !/^[：:]{2,}/.test(t));
      if (!designConfirmedSilent && embeddedCjk.length) {
        audio.body = `${embeddedCjk.map((t) => `"${t}"`).join("\n")}\n口型同步开启。`.trim();
        changes.push("audio_keep_embedded_cjk");
      } else {
  ...
  if (camera) {
    ...
    const needsNormalize =
      ...
      (targetDur != null &&
        durations.length === 1 &&
        Math.abs(Number(durations[0]) - targetDur) > 0.5) ||
      (targetDur != null && durations.length === 0 && !/(?:时长|duration)\s*\d/i.test(cam));
    if (needsNormalize && targetDur != null) {
      ...
      cam = cam
        ...
        .replace(/\s+/g, " ")
        .trim();
      cam = `${cam.replace(/[，,]\s*$/, "")}${cam ? "，" : ""}时长 ${targetDur}s`.replace(/\s+/g, " ").trim();
      changes.push(`camera_duration_${targetDur}`);
