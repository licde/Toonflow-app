...
const BLOCK_TO_TRIGGER: Record<string, string> = {
  ...
  "DEX-LIT-CONTACT-XOR": "lit_detail_contact_xor",
  "DEX-LIT-ANCHOR": "lit_detail_anchor",
  "DEX-LIT-EXPR": "lit_detail_expr",
  "DEX-PROP-CONT": "prop_continuity",
  "DEX-PROP-IN-FRAME": "still_prop_missing",
  "STILL-CONTACT-HANDOFF": "still_video_contact_handoff",
  "VID-CONTACT-BEATS": "vid_contact_beats",
  ...
  "VIDEO-PROMPT-STALE": "video_prompt_stale",
  "DEX-CAM-FIT": "cam_fit",
...
export function buildBurnGateEnvelope(
  ...
  let ctaOverride: string | undefined = opts?.primary?.ctaLabel;
  if (!userMessageOverride && triggers.includes("dialogue_hash_mismatch")) {
    ...
    if (nextStep === "soft_patch") ctaOverride = ctaOverride ?? env.ctaLabel;
  }
  // img_cref: keep 定妆 CTA (batch_still default is now 静照)
  if (!userMessageOverride && triggers.includes("img_cref_missing") && nextStep === "batch_still") {
    userMessageOverride = "角色定妆图还没有";
    ctaOverride = ctaOverride ?? "去生成定妆";
  }
  // Prefer concrete gate message over generic nextStep copy (weak/missing/sheetLeak)
  if (
    !userMessageOverride &&
    nextStep === "batch_still" &&
    (triggers.includes("still_firstframe_weak") ||
      triggers.includes("still_firstframe_dirty") ||
      triggers.includes("img_still_weak"))
  ) {
    const msg = blocks.map((b) => b.message).find((m) => String(m ?? "").trim());
    if (msg) userMessageOverride = msg;
    if (!ctaOverride && /拼版|多格|四视/i.test(String(msg))) ctaOverride = "禁拼版重抽";
    else if (!ctaOverride) ctaOverride = "去生成静照";
  }
