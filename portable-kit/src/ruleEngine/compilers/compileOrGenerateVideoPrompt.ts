...
export function applyModeDialect(prompt: string, modeId: string, existingPrompt?: string): string {
  ...
  if (id === "startEndRequired" || id === "endFrameOptional" || id === "startFrameOptional" || id === "firstLastFrame") {
    ...
    // startEndRequired / firstLastFrame: mode contract requires END_FRAME scaffold.
    // Other FLF modes: only emit END_FRAME when end cue already present.
    const dualFrameRequired = id === "startEndRequired" || id === "firstLastFrame";
    const hasEndCue =
      dualFrameRequired ||
      /END_FRAME\s*:/i.test(next) ||
      /endFrame|尾帧|结束帧|--eref/i.test(next) ||
      id === "endFrameOptional";
    if (hasEndCue && !/END_FRAME\s*:/i.test(next)) {
