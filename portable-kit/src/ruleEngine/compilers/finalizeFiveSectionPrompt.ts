...
export function finalizeFiveSectionPrompt(input: FinalizeFiveSectionInput): FinalizeFiveSectionResult {
  ...
  // --- Camera: strip duration Ns, enforce single numeric duration ---
  {
    ...
    const durations = [...durationsZh, ...durationsEn];
    const needsCamFix =
      /duration\s*Ns\b/i.test(cam) ||
      durations.length > 1 ||
      (dur != null && durations.length === 1 && Math.abs(Number(durations[0]) - dur) > 0.5);
    if (needsCamFix) {
      cam = cam
        ...
        .replace(/\s+/g, " ")
        .trim();
      if (dur != null) cam = `${cam.replace(/[，,]\s*$/, "")}${cam ? "，" : ""}时长 ${dur}s，单次连续镜头。`;
      cam = cam.replace(/,\s*,/g, ",").replace(/\s+/g, " ").trim() || `近景，轻微运镜，时长 ${dur ?? 4}s，单次连续镜头。`;
  ...
  // --- Audio: placeholders / fake --- / silence+dialogue XOR / ban "1." stub ---
  {
    ...
    if (stubOne) {
      ...
      conflicts.push("VP-PLACEHOLDER_AUDIO");
    } else if (!hasLines && audio.trim()) {
      // Duration pseudo / orphan lip when design has no literary dialogue
      const stripped = audio
        .split(/\n/)
        .filter((line) => {
          const t = line.replace(/^["「]|["」]$/g, "").trim();
          if (!t) return false;
          if (/^[:：\s.\-_·•]*\d+(\.\d+)?\s*s$/i.test(t)) return false;
          return !isNonLiteraryDialogueKey(t);
        })
        .join("\n")
        .replace(/口型同步开启。?/g, "")
        .trim();
      if (stripped !== audio.trim()) {
        audio = stripped && !/无对白/.test(stripped) ? `无对白。仅环境音效。\n${stripped}`.trim() : stripped || "无对白。仅环境音效。";
        prompt = injectSection(prompt, "Audio", audio);
        changes.push("finalize_audio_strip_pseudo");
        conflicts.push("VP-PLACEHOLDER_AUDIO");
      }
