  ...
  () => {
    ...
    async function resolveUrls(items) {
      ...
      if (needResolve.length) {
        try {
          ...
          if (rawData && typeof rawData === "object" && !Array.isArray(rawData) && rawData.data && typeof rawData.data === "object") {
    ...
    function initCacheFromTrackList(projectId, scriptId, trackList) {
      trackList.forEach((track) => {
        ...
        const byKey = new Map(
          existing.map((c) => [`${c.id ?? ""}:${c.sources ?? "storyboard"}`, c])
        );
        let dirty = false;
        for (const m of track.medias ?? []) {
          if (!m?.src || m.id == null) continue;
          const sources = m.sources ?? "storyboard";
