...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    async function handleBatchGenerateImage() {
      ...
      batchGenerationShow.value = false;
      try {
        const res = await instance.post("/assetsGenerate/batchGenerateImageAssets", {
          projectId: project.value?.id,
          model: selectValue.value,
          resolution: resolution.value,
          concurrentCount: otherSetting.value.assetsBatchGenereateSize,
        ...
        if (Array.isArray(data.deferred) && data.deferred.length) {
          window.$message.info(
            data.message || `已受理 ${data.accepted ?? 0}；暂缓 ${data.deferred.length}（请先批量生成提示词）`
          );
        }
      } catch (e) {
        ...
        if (Array.isArray(data) && data.length) {
          ...
          data.forEach((item) => {
            ...
            if (target) {
              ...
              if (item.promptErrorReason !== void 0) target.promptErrorReason = item.promptErrorReason;
      ...
      try {
        ...
        if (Array.isArray(data) && data.length) {
          ...
          data.forEach(
            (item) => {
              ...
              if (target) {
                ...
                if (item.errorMessage) target.errorReason = item.errorMessage;
                if (item.nextStep) target.nextStep = item.nextStep;
                if (item.state === "生成失败") {
                  failCount++;
          ...
          if (failCount > 0 && generatingData.value.length === 0) {
            window.$message.warning(
              `图片失败 ${failCount} 条：${failReasons.slice(0, 3).join("；")}${failReasons.length > 3 ? "…" : ""}${healHint ? "（可用「修复并重试失败项」）" : ""}`
            );
          }
        }
      } catch (e) {
    ...
    async function handleHealAndRetry() {
      ...
      const promptFailed = all.filter(
        (a) => a.promptState === "生成失败" || String(a.promptErrorReason ?? "").includes("complete_failed")
      );
                            ...
                            createVNode(_component_t_popup, { placement: "bottom" }, {
                              content: withCtx(() => [
                                createBaseVNode("div", _hoisted_6, [
                                  ...
                                  createBaseVNode("div", { class: "generateImage" }, [
                                    createBaseVNode("span", { onClick: handleHealAndRetry }, "修复并重试失败项")
                                  ])
                                ])
                              ]),
                              default: withCtx(() => [
...
const AssetsView = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-eecf8c83"]]);
