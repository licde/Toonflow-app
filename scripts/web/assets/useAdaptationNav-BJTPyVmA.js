import { o as onMounted, b as onUnmounted, aj as shallowRef, c as computed, r as ref, bD as defineStore, w as watch, bX as useThrottleFn, bR as useRouter } from './vue-vendor-Byo5TD6r.js';
import { i as instance } from './axios-DoLZCC01.js';
import { p as projectStore, s as settingStore } from './index-Dj17DntQ.js';
  ...
  return defineStore(`productionAgent-${projectId}`, () => {
    ...
    async function batchGenerateStoryboard(allIds, compulsory = false) {
      try {
        ...
        window.$message.error(e?.response?.data?.data?.userMessage || e?.message);
        throw e;
      }
    }
    async function batchComposeStillPrompts(allIds, mode = "full") {
      ...
      try {
        ...
        records.forEach((record) => {
          ...
          if (item) {
            ...
            if (record.stillQuality != null) item.stillQuality = record.stillQuality;
...
function useAdaptationNav() {
  ...
  function goDesignStage(reverseTarget, trigger) {
    if (!requireProjectOrRedirect(router)) return;
  ...
  function goProduction(scriptId) {
    if (!requireProjectOrRedirect(router)) return;
    if (scriptId) {
      useProductionAgentStore().episodesId = scriptId;
    }
    router.push({
      path: "/production",
      query: scriptId ? { scriptId: String(scriptId) } : void 0
    });
  }
  return { goAdaptation, goOriginal, goExternalRevision, goProduction, goDesignStage, STAGE_TAB, OPEN_SCRIPT_IMPORT_KEY };
}

export { useChat as a, useProductionAgentStore as b, useAdaptationNav as u };
