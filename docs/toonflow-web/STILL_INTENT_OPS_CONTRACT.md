...
- FE：`LitDetailDebtBar` 支持批准增强 / 拆镜 / 人审 / **presentationFork 双路径** / 手改；类型见 `docs/toonflow-web/types/stillIntentOps.ts`；`stillQuality.shouldBlockSilentStillRegen` **恒 false**；CTA 永不 `blocksGenerate`（缺定妆=`补定妆并继续生成`；`PROMPT-FIDELITY`=`增强锚点并生成`）；finding→反推 untilClear；`requireFixBeforeBurn`≠门闩；触达 toast 合并时 passed 优先于陈旧 CONTRACT
